...
class OlodbServer {
    ...
    //
    allow (action, agent, collection, docId) {
        return new Promise((resolve, reject) => {
            var userName = agent.custom.userName;
            ...
            if (agent === this.rootConnection.agent) {
                ...
                this.getUserRights(agent.custom.userName, collection, docId)
                .then((userRights) => {
                    if (userRights >= rights[action]) {
                        logger.debug(`[olodb]: ${action} permission GRANTED to ${userName} on ${path}.`);
                        resolve();
                    }
                    else {
                        logger.debug(`[olodb]: ${action} permission DENIED to ${userName} on ${path}.`);
    ...
    allowRead (agent, collection, docId) {
        return this.allow('READ', agent, collection, docId);
    }
    
    allowWrite (agent, collection, docId) {
        return this.allow('WRITE', agent, collection, docId);
    }
}
