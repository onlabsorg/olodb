...
Store.Document = class extends olojs.Store.Document {
    ...
    __fetch () {
        const self = this;
        return co(function* () {
            const role = yield self.store.__getUserRole(self._collection, self._docName);
            if (role < olojs.roles.READER) throw new olojs.errors.ReadPermissionError(self.id);            
    ...
    __create (content) {
        ...
        return co(function* () {
            const role = yield self.store.__getUserRole(self._collection, self._docName);
            if (role < olojs.roles.OWNER) throw new olojs.errors.WritePermissionError(self.id);                        
...
Store.User = class extends olojs.Store.User {}
